...
`;
  ...
  async generateOptimizedContent(data: {
    originalContent: {
      headline: string;
      subheadline?: string;
      ctaText?: string;
    ...
    try {
      ...
      const systemPrompt = `
You are an expert in conversion rate optimization (CRO) for websites.
Your task is to rewrite the landing page content to improve conversions based on the identified weaknesses and recommendations.
The optimized content should be clear, compelling, and aligned with best practices for high-converting websites.

IMPORTANT: Generate only the optimized content without additional explanation in your main output. Your response should be in JSON format containing:
- headline: The optimized headline (compelling, clear, benefit-focused)
- subheadline: The optimized subheadline (expanded value proposition, addressing pain points)
- ctaText: The optimized call-to-action text (action-oriented, clear, value-based)
- content: A condensed version of the optimized main content (key points only)
- explanation: A brief explanation of your changes and how they address the weaknesses
`;

      const userPrompt = `
Please optimize this landing page content based on the identified weaknesses and recommendations:

ORIGINAL CONTENT:
Headline: ${data.originalContent.headline}
Subheadline: ${data.originalContent.subheadline || "(none)"}
CTA Text: ${data.originalContent.ctaText || "(none)"}
Content: ${data.originalContent.content || "(none)"}
...
Format your response as a JSON object with headline, subheadline, ctaText, content, and explanation fields. 
Make sure all content is concise, compelling, and optimized for conversions.
`;
      ...
      try {
        ...
        return {
          headline: optimizedContent.headline || data.originalContent.headline,
          subheadline:
            optimizedContent.subheadline ||
            data.originalContent.subheadline ||
            "",
          ctaText:
            optimizedContent.ctaText || data.originalContent.ctaText || "",
          content:
            optimizedContent.content || data.originalContent.content || "",
          explanation:
            optimizedContent.explanation || "No explanation provided.",
        };
      } catch (jsonError) {
        ...
        return {
          headline: data.originalContent.headline,
          subheadline: data.originalContent.subheadline || "",
          ctaText: data.originalContent.ctaText || "",
          content: data.originalContent.content || "",
          explanation:
            "Failed to generate optimized content. Please try again.",
        };
      }
